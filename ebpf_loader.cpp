...
std::unordered_multimap<std::string, Hook> Hooks;

bool ReadAndApplyConfig()
{
    std::unordered_map<std::string, bool> config;

    if (std::ifstream file("sw_test_ebpf_loader.cfg"); file.is_open())
    {
        std::string line;
        ...
        while (getline(file, line))
        {
            ...
            std::string value;
            ss >> eventName >> value;
            config.emplace(std::move(eventName), value == "enable");
        }
    }

    for (const auto& [event, enable] : config)
    {
        const auto& range = Hooks.equal_range(event);
        for (auto it = range.first; range.second != it; ++it)
        {
            ...
            if (!hook.link && enable)
            {
                hook.link = bpf_program__attach(hook.prog);

                if (!hook.link)
                {
                    std::cerr
                        << "ERR: couldn't attach "
                        << bpf_program__name(hook.prog) << " to "
                        << bpf_program__section_name(hook.prog) << ": "
                        << strerror(errno)
                        << std::endl;
                ...
                std::cout
                    << "Hook (" << it->first << ":" << hook.name << ") "
                    << "seccessfuly attached." << std::endl;
            }
            else if (hook.link && !enable)
            {
                if (bpf_link__destroy(hook.link))
                {
                    std::cerr
                        << "ERR: couldn't detach "
                        << bpf_program__name(hook.prog) << " to "
